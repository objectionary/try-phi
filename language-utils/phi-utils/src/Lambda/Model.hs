...
matchObjs eqs o1 o2
  | sort (InsOrdHashMap.keys o1) == sort (InsOrdHashMap.keys o2) =
    sequence
      (InsOrdHashMap.unionWith matchTerms'' (Right <$> o1) (Right <$> o2))
...
matchTerms' eqs tl tr =
  foldr1 orEither $
  interleaveMany
    [ matchTermsNaive eqs tl' <$> take 10 (whnfSteps tr)
    | tl' <- take 10 $ whnfSteps tl
    ]
...
matchTermsNaive eqs tl tr
  ...
  | otherwise =
    case (tl, tr) of
      (Var i, Var j) -> Var <$> matchEq i j
      (Abs body1, Abs body2) -> Abs <$> go body1 body2
      (App fun1 arg1, App fun2 arg2) -> App <$> go fun1 fun2 <*> go arg1 arg2
      (Dot t1 a1, Dot t2 a2) -> Dot <$> go t1 t2 <*> matchEq a1 a2
      (Obj o1, Obj o2) -> Obj <$> matchObjs eqs o1 o2
      (With t1 o1, With t2 o2) -> With <$> go t1 t2 <*> matchObjs eqs o1 o2
      (Cat l1 r1, Cat l2 r2) -> Cat <$> go l1 l2 <*> go r1 r2
      (Fix t1, Fix t2) -> Fix <$> go t1 t2
      (_, _) -> Left (tl, tr)
  where
